# File Encryption & Decryption Tool (AES-256-GCM)

## 1. Introduction
This is a command-line tool built in Java to securely encrypt and decrypt any file using **AES-256-GCM**.  
A strong key is derived from the user’s password using **PBKDF2WithHmacSHA256**, ensuring confidentiality and integrity.  
The encrypted output file contains the salt, IV, ciphertext, and authentication tag in one packaged format.

---

## 2. Tech Stack
- **Java 17**
- **AES-256-GCM (JCA)**
- **PBKDF2WithHmacSHA256**
- **Maven**

---

## 3. Project Structure
```
src/
└── main/
    └── java/
        └── com/
            └── example/
                └── filecrypto/
                    ├── Main.java          # CLI for encryption/decryption
                    ├── CryptoUtils.java   # AES-GCM logic + packaging
                    └── KeyUtils.java      # PBKDF2 key derivation + salt generation

pom.xml               # Maven configuration
```

---

## 4. Important Notes
- If you lose your password, **your data cannot be recovered**.  
- AES-GCM provides authentication—any modification to encrypted data will cause decryption failure.  
- Encrypted file format:
  ```
  [saltLength][salt][ivLength][iv][ciphertext][tag]
  ```
- You can optionally provide a custom salt in HEX (advanced usage).

---

## 5. How to Use

### **Build the Project**
```
mvn clean package
```

This generates:
```
target/file-crypto-cli-1.0.0.jar
```

---

### **Encrypt a File**
```
java -jar target/file-crypto-cli-1.0.0.jar encrypt input.txt encrypted.bin --password "mypassword"
```

---

### **Decrypt a File**
```
java -jar target/file-crypto-cli-1.0.0.jar decrypt encrypted.bin output.txt --password "mypassword"
```

---

### **Sample Test**
Create a test file:
```
echo "Hello World" > test.txt
```

Encrypt:
```
java -jar target/file-crypto-cli-1.0.0.jar encrypt test.txt secure.bin --password "1234"
```

Decrypt:
```
java -jar target/file-crypto-cli-1.0.0.jar decrypt secure.bin decrypted.txt --password "1234"
```

Verify:
```
cat decrypted.txt
```

---

## 6. License
This project is open-source and free to use.
